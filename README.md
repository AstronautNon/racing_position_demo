...
tools/
  ...
  probe_contour_axis.py      通用轮廓轴（与车色无关）对人工真值的验收（见 §14.7.1）
...
- [ ] **标 video02 的前 15 帧当验证集**（最高优先）—— 用来验证通用轮廓轴能否替代人工标注，
      决定剩下近 200 帧要不要逐帧标。理由见 §14.7.1
- [ ] 拿到第二段素材的真值后，把轮廓轴接入 `pipeline` 作为 `axis_src=cv` 并复验
- [ ] 继续标注其余素材（按 §14.7 的优先级：video02 → video03 → video01；
      video07/08 抽检十几帧即可）
